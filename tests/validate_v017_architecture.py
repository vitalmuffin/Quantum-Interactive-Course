#!/usr/bin/env python3
"""Architecture and build invariants for the v0.17 interaction-correction release."""
...
need(config.get('courseVersion')=='0.17','canonical config version is not 0.17')
...
need('data/course_overview.bundle.js?v=0.17' in index,'index does not load the lightweight overview data')
need('vendor/course-data-loader.js?v=0.17' in index,'index does not configure lazy full-detail loading')
need('data/course_data.bundle.js?v=0.17' not in index,'full paper data is still eagerly loaded by index')
...
shared=['vendor/course-config.js?v=0.17','vendor/qm-state.js?v=0.17','vendor/page-api.js?v=0.17','vendor/course-shell.js?v=0.17','vendor/course-enhancements.js?v=0.17']
...
print('OK: v0.17 canonical data, migration, iframe API, modular CSS, split loading, math modes, quizzes, and build invariants passed.')
