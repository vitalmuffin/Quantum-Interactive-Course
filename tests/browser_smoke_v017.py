...
async def shell_fixture(browser,viewport):
    ...
    await page.set_content('''<!doctype html><html lang="de" data-theme="dark"><body data-course-page="index.html">
      ...
      <main id="main-content"><section id="intro" style="min-height:900px"><h1>Fixture</h1><input id="dragRange" type="range" min="0" max="10" step="1" value="0" style="width:300px"></section></main>
    </body></html>''')
    await page.evaluate("""() => {
    ...
    # Shared pointer fallback must make range thumbs continuously draggable.
    await page.evaluate("window.__rangeInputs=0;document.querySelector('#dragRange').addEventListener('input',()=>window.__rangeInputs++)")
    box=await page.locator('#dragRange').bounding_box()
    if not box: raise AssertionError('range control has no layout box')
    await page.mouse.move(box['x']+4,box['y']+box['height']/2)
    await page.mouse.down()
    await page.mouse.move(box['x']+box['width']*.92,box['y']+box['height']/2,steps=8)
    await page.mouse.up()
    range_value=float(await page.locator('#dragRange').input_value())
    range_events=await page.evaluate('window.__rangeInputs')
    if range_value<8 or range_events<2: raise AssertionError(f'range drag failed: value={range_value}, events={range_events}')
...
async def main():
    async with async_playwright() as p:
        ...
        await browser.close()
    print('OK: Chromium rendered all three math modes, generated one shell/rail, exposed advanced quizzes, dragged shared range controls, and passed mobile off-canvas interaction.')
